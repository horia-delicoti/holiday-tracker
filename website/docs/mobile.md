...
| **A long ledger folds** | A trip with 19 lines made the trip page 2844px — 3.4 screens, nearly all of it ledger. Below 700px it shows **nine rows** and then *"Show all 19 lines"*; the page is 2030px folded. The hidden rows stay in the DOM and are hidden by a class on the container, so opening is one class change rather than another render, and a day header folds away with its own rows rather than standing over nothing. The choice is keyed to the trip id, not a boolean, so unfolding one ledger does not unfold the next trip you open. Desktop never folds — the list already scrolls inside its own 520px box. |
